...
void main() {
  // Normalize X and Y independently so the wave frequency and amplitude scale perfectly
  // to the container's box without squishing into a million tight waves on wide aspect ratios.
  vec2 uv;
  uv.x = (gl_FragCoord.xy.x - 0.5 * uResolution.x) / uResolution.x * 2.0; // -1.0 to 1.0
  uv.y = (gl_FragCoord.xy.y - 0.5 * uResolution.y) / uResolution.y * 2.0; // -1.0 to 1.0
  uv /= max(uScale, 0.0001);

  float e = 0.06 + uIntensity * 0.94;
  // Envelope spanning exactly edge to edge
  float env = pow(max(cos(uv.x * PI * 0.5), 0.0), uTaper);
